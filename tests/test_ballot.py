...
def ballot(scope="module"):
    return Ballot.deploy(proposals, {"from": accounts[0]})


def test_correct_chairperson(ballot):
    assert ballot.creator() == accounts[0]


def test_has_proposals(ballot):
    assert ballot.proposals(0)[0] == proposals[0]
    assert ballot.proposals(1)[0]== proposals[1]
    assert ballot.proposals(2)[0] == proposals[2]
